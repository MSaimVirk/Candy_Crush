#include <windows.h>
#include <math.h>

void gotoxy(int x, int y)
{
	COORD coord;
	coord.X = x;
	coord.Y = y;
	SetConsoleCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE), coord);
}

void myLine(int x1, int y1, int x2, int y2, int color) //use three 3 integers if you want colored lines.
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);

	//change the color by changing the values in RGB (from 0-255) to get shades of gray. For other colors use 3 integers for colors.
	HPEN pen = CreatePen(PS_SOLID, 2, RGB(color, color, color)); //2 is the width of the pen
	SelectObject(device_context, pen);
	MoveToEx(device_context, x1, y1, NULL);
	LineTo(device_context, x2, y2);
	ReleaseDC(console_handle, device_context);
	DeleteObject(pen);

}


// This function checks if any of the 4 cursor keys are pressed. 
// If any cursor key is pressed, then the function returns true, and whichKey identifies which of the 4 cursor keys is pressed.
// whichkey is assigned following values if any cursor key is pressed. 1 for left, 2 for up, 3 for right and 4 for left.
bool isCursorKeyPressed(int& whichKey)   //whichKey passed as reference.... 
{
	char key;
	key = GetAsyncKeyState(37);
	if (key == 1)
	{
		whichKey = 1;		// 1 if left key is pressed 
		return true;
	}
	key = GetAsyncKeyState(38);
	if (key == 1)
	{

		whichKey = 2;	 	// 2 if up key is pressed
		return true;
	}

	key = GetAsyncKeyState(39);
	if (key == 1)
	{

		whichKey = 3;       // 3 if right key is pressed
		return true;
	}
	key = GetAsyncKeyState(40);
	if (key == 1)
	{

		whichKey = 4;       // 4 if down key is pressed
		return true;
	}
	key = GetAsyncKeyState(13);
	if (key == 1)
	{

		whichKey = 5;      // 5 if ENTER key is pressed
		return true;
	}
	return false;
}

void myRectangle(int x1, int y1, int x2, int y2, int R, int G, int B)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);

	//change the color by changing the values in RGB (from 0-255)
	HPEN pen = CreatePen(PS_SOLID, 2, RGB(R, G, B));
	SelectObject(device_context, pen);
	HBRUSH brush = ::CreateSolidBrush(RGB(0, 0, 0)); //Fill color can also be passed as parameter to the function!!!

	SelectObject(device_context, brush);

	Rectangle(device_context, x1, y1, x2, y2);
	ReleaseDC(console_handle, device_context);
	DeleteObject(pen);
	DeleteObject(brush);
}

void myRectangle1(int x1, int y1, int x2, int y2, int R, int G, int B)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);

	//change the color by changing the values in RGB (from 0-255)
	HPEN pen = CreatePen(PS_SOLID, 2, RGB(R, G, B));
	SelectObject(device_context, pen);
	HBRUSH brush = ::CreateSolidBrush(RGB(0, 0, 0)); //Fill color can also be passed as parameter to the function!!!

	SelectObject(device_context, brush);
	for (int i = 0; i < 5; i++)
	{
		Rectangle(device_context, x1, y1, x2, y2);
		x1 = x1 + 6, x2 = x2 - 6, y1 = y1 + 6, y2 = y2 - 6;
	}

	ReleaseDC(console_handle, device_context);
	DeleteObject(pen);
	DeleteObject(brush);
}

void myRectangle2(int x1, int y1, int x2, int y2, int R, int G, int B)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);

	//change the color by changing the values in RGB (from 0-255)
	HPEN pen = CreatePen(PS_SOLID, 2, RGB(R, G, B));
	SelectObject(device_context, pen);
	HBRUSH brush = ::CreateSolidBrush(RGB(0, 0, 0)); //Fill color can also be passed as parameter to the function!!!

	SelectObject(device_context, brush);

	for (int i = 0; i < 16; i++)
	{
		Rectangle(device_context, x1, y1, x2, y2);
		x1 = x1 + 1, x2 = x2 - 1, y1 = y1 + 1, y2 = y2 - 1;
	}
	ReleaseDC(console_handle, device_context);
	DeleteObject(pen);
	DeleteObject(brush);
}

void myRectangle3(int x1, int y1, int x2, int y2, int R, int G, int B)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);

	//change the color by changing the values in RGB (from 0-255)
	HPEN pen = CreatePen(PS_SOLID, 2, RGB(R, G, B));
	SelectObject(device_context, pen);
	HBRUSH brush = ::CreateSolidBrush(RGB(0, 0, 0)); //Fill color can also be passed as parameter to the function!!!

	SelectObject(device_context, brush);

	for (int i = 0; i < 25; i++)
	{
		Rectangle(device_context, x1, y1, x2, y2);
		x1 = x1 + 1, x2 = x2 - 1, y1 = y1 + 1, y2 = y2 - 1;
	}
	ReleaseDC(console_handle, device_context);
	DeleteObject(pen);
	DeleteObject(brush);
}

void myEllipse(int x1, int y1, int x2, int y2, int R, int G, int B)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);

	//change the color by changing the values in RGB (from 0-255)
	HPEN pen = CreatePen(PS_SOLID, 2, RGB(R, G, B));
	SelectObject(device_context, pen);
	HBRUSH brush = ::CreateSolidBrush(RGB(0, 0, 0));  //Fill color is black
	SelectObject(device_context, brush);
	Ellipse(device_context, x1, y1, x2, y2);
	ReleaseDC(console_handle, device_context);
	DeleteObject(pen);
	DeleteObject(brush);


}

void myEllipse1(int x1, int y1, int x2, int y2, int R, int G, int B)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);

	//change the color by changing the values in RGB (from 0-255)
	HPEN pen = CreatePen(PS_SOLID, 2, RGB(R, G, B));
	SelectObject(device_context, pen);
	HBRUSH brush = ::CreateSolidBrush(RGB(0, 0, 0));  //Fill color is black
	SelectObject(device_context, brush);
	for (int i = 0; i < 5; i++)
	{
		Ellipse(device_context, x1, y1, x2, y2);
		x1 = x1 + 5, x2 = x2 - 5, y1 = y1 + 5, y2 = y2 - 5;
	}
	ReleaseDC(console_handle, device_context);
	DeleteObject(pen);
	DeleteObject(brush);


}

void myEllipse2(int x1, int y1, int x2, int y2, int R, int G, int B)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);

	//change the color by changing the values in RGB (from 0-255)
	HPEN pen = CreatePen(PS_SOLID, 2, RGB(R, G, B));
	SelectObject(device_context, pen);
	HBRUSH brush = ::CreateSolidBrush(RGB(0, 0, 0));  //Fill color is black
	SelectObject(device_context, brush);
	for (int i = 0; i < 16; i++)
	{
		Ellipse(device_context, x1, y1, x2, y2);
		x1 = x1 + 1, x2 = x2 - 1, y1 = y1 + 1, y2 = y2 - 1;
	}
	ReleaseDC(console_handle, device_context);
	DeleteObject(pen);
	DeleteObject(brush);


}

void myEllipse3(int x1, int y1, int x2, int y2, int R, int G, int B)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);

	//change the color by changing the values in RGB (from 0-255)
	HPEN pen = CreatePen(PS_SOLID, 2, RGB(R, G, B));
	SelectObject(device_context, pen);
	HBRUSH brush = ::CreateSolidBrush(RGB(0, 0, 0));  //Fill color is black
	SelectObject(device_context, brush);
	for (int i = 0; i < 25; i++)
	{
		Ellipse(device_context, x1, y1, x2, y2);
		x1 = x1 + 1, x2 = x2 - 1, y1 = y1 + 1, y2 = y2 - 1;
	}
	ReleaseDC(console_handle, device_context);
	DeleteObject(pen);
	DeleteObject(brush);


}

void myDiamond(int x1, int y1, int x2, int y2, int R, int G, int B)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);

	//change the color by changing the values in RGB (from 0-255)
	HPEN pen = CreatePen(PS_SOLID, 2, RGB(R, G, B));
	SelectObject(device_context, pen);
	HBRUSH brush = ::CreateSolidBrush(RGB(R, G, B));
	SelectObject(device_context, brush);
	int midx = (x1 + x2) / 2;
	int midy = (y1 + y2) / 2;
	MoveToEx(device_context, midx, y1, NULL);
	LineTo(device_context, x1, midy);
	LineTo(device_context, midx, y2);
	LineTo(device_context, x2, midy);
	LineTo(device_context, midx, y1);
	ReleaseDC(console_handle, device_context);
	DeleteObject(pen);
	DeleteObject(brush);
}

void myDiamond1(int x1, int y1, int x2, int y2, int R, int G, int B)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);

	//change the color by changing the values in RGB (from 0-255)
	HPEN pen = CreatePen(PS_SOLID, 2, RGB(R, G, B));
	SelectObject(device_context, pen);
	HBRUSH brush = ::CreateSolidBrush(RGB(R, G, B));
	SelectObject(device_context, brush);
	int midx = (x1 + x2) / 2;
	int midy = (y1 + y2) / 2;
	for (int i = 0; i < 5; i++)
	{
		MoveToEx(device_context, midx, y1, NULL);
		LineTo(device_context, x1, midy);
		LineTo(device_context, midx, y2);
		LineTo(device_context, x2, midy);
		LineTo(device_context, midx, y1);
		x1 = x1 + 6, x2 = x2 - 6, y1 = y1 + 6, y2 = y2 - 6;
	}
	ReleaseDC(console_handle, device_context);
	DeleteObject(pen);
	DeleteObject(brush);
}

void myDiamond2(int x1, int y1, int x2, int y2, int R, int G, int B)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);

	//change the color by changing the values in RGB (from 0-255)
	HPEN pen = CreatePen(PS_SOLID, 2, RGB(R, G, B));
	SelectObject(device_context, pen);
	HBRUSH brush = ::CreateSolidBrush(RGB(R, G, B));
	SelectObject(device_context, brush);
	int midx = (x1 + x2) / 2;
	int midy = (y1 + y2) / 2;
	for (int i = 0; i < 16; i++)
	{
		MoveToEx(device_context, midx, y1, NULL);
		LineTo(device_context, x1, midy);
		LineTo(device_context, midx, y2);
		LineTo(device_context, x2, midy);
		LineTo(device_context, midx, y1);
		x1 = x1 + 1, x2 = x2 - 1, y1 = y1 + 1, y2 = y2 - 1;
	}
	ReleaseDC(console_handle, device_context);
	DeleteObject(pen);
	DeleteObject(brush);
}

void myDiamond3(int x1, int y1, int x2, int y2, int R, int G, int B)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);

	//change the color by changing the values in RGB (from 0-255)
	HPEN pen = CreatePen(PS_SOLID, 2, RGB(R, G, B));
	SelectObject(device_context, pen);
	HBRUSH brush = ::CreateSolidBrush(RGB(R, G, B));
	SelectObject(device_context, brush);
	int midx = (x1 + x2) / 2;
	int midy = (y1 + y2) / 2;
	for (int i = 0; i < 25; i++)
	{
		MoveToEx(device_context, midx, y1, NULL);
		LineTo(device_context, x1, midy);
		LineTo(device_context, midx, y2);
		LineTo(device_context, x2, midy);
		LineTo(device_context, midx, y1);
		x1 = x1 + 1, x2 = x2 - 1, y1 = y1 + 1, y2 = y2 - 1;
	}
	ReleaseDC(console_handle, device_context);
	DeleteObject(pen);
	DeleteObject(brush);
}

void myTriangle(int x1, int y1, int x2, int y2, int R, int G, int B)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);

	//change the color by changing the values in RGB (from 0-255)
	HPEN pen = CreatePen(PS_SOLID, 2, RGB(R, G, B));
	SelectObject(device_context, pen);
	int mid = (x1 + x2) / 2;
	MoveToEx(device_context, mid, y1, NULL);
	LineTo(device_context, x1, y2);
	LineTo(device_context, x2, y2);
	LineTo(device_context, mid, y1);
	ReleaseDC(console_handle, device_context);
	DeleteObject(pen);
}

void myTriangle1(int x1, int y1, int x2, int y2, int R, int G, int B)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);

	//change the color by changing the values in RGB (from 0-255)
	HPEN pen = CreatePen(PS_SOLID, 2, RGB(R, G, B));
	SelectObject(device_context, pen);
	int mid = (x1 + x2) / 2;
	for (int i = 0; i < 5; i++)
	{
		MoveToEx(device_context, mid, y1, NULL);
		LineTo(device_context, x1, y2);
		LineTo(device_context, x2, y2);
		LineTo(device_context, mid, y1);
		x1 = x1 + 6, x2 = x2 - 6, y1 = y1 + 8, y2 = y2 - 4;
	}
	ReleaseDC(console_handle, device_context);
	DeleteObject(pen);
}

void myTriangle2(int x1, int y1, int x2, int y2, int R, int G, int B)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);

	//change the color by changing the values in RGB (from 0-255)
	HPEN pen = CreatePen(PS_SOLID, 2, RGB(R, G, B));
	SelectObject(device_context, pen);
	int mid = (x1 + x2) / 2;
	for (int i = 0; i < 16; i++)
	{
		MoveToEx(device_context, mid, y1, NULL);
		LineTo(device_context, x1, y2);
		LineTo(device_context, x2, y2);
		LineTo(device_context, mid, y1);
		x1 = x1 + 1, x2 = x2 - 1, y1 = y1 + 1, y2 = y2 - 1;
	}
	ReleaseDC(console_handle, device_context);
	DeleteObject(pen);
}

void myTriangle3(int x1, int y1, int x2, int y2, int R, int G, int B)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);

	//change the color by changing the values in RGB (from 0-255)
	HPEN pen = CreatePen(PS_SOLID, 2, RGB(R, G, B));
	SelectObject(device_context, pen);
	int mid = (x1 + x2) / 2;
	for (int i = 0; i < 25; i++)
	{
		MoveToEx(device_context, mid, y1, NULL);
		LineTo(device_context, x1, y2);
		LineTo(device_context, x2, y2);
		LineTo(device_context, mid, y1);
		x1 = x1 + 1, x2 = x2 - 1, y1 = y1 + 1, y2 = y2 - 1;
	}
	ReleaseDC(console_handle, device_context);
	DeleteObject(pen);
}

void myUDTriangle(int x1, int y1, int x2, int y2, int R, int G, int B)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);

	//change the color by changing the values in RGB (from 0-255)
	HPEN pen = CreatePen(PS_SOLID, 2, RGB(R, G, B));
	SelectObject(device_context, pen);
	int mid = (x1 + x2) / 2;
	MoveToEx(device_context, x1, y1, NULL);
	LineTo(device_context, x2, y1);
	LineTo(device_context, mid, y2);
	LineTo(device_context, x1, y1);
	ReleaseDC(console_handle, device_context);
	DeleteObject(pen);
}

void myUDTriangle1(int x1, int y1, int x2, int y2, int R, int G, int B)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);

	//change the color by changing the values in RGB (from 0-255)
	HPEN pen = CreatePen(PS_SOLID, 2, RGB(R, G, B));
	SelectObject(device_context, pen);
	int mid = (x1 + x2) / 2;
	for (int i = 0; i < 5; i++)
	{
		MoveToEx(device_context, x1, y1, NULL);
		LineTo(device_context, x2, y1);
		LineTo(device_context, mid, y2);
		LineTo(device_context, x1, y1);
		x1 = x1 + 6, x2 = x2 - 6, y1 = y1 + 4, y2 = y2 - 8;
	}
	ReleaseDC(console_handle, device_context);
	DeleteObject(pen);
}

void myUDTriangle2(int x1, int y1, int x2, int y2, int R, int G, int B)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);

	//change the color by changing the values in RGB (from 0-255)
	HPEN pen = CreatePen(PS_SOLID, 2, RGB(R, G, B));
	SelectObject(device_context, pen);
	int mid = (x1 + x2) / 2;
	for (int i = 0; i < 16; i++)
	{
		MoveToEx(device_context, x1, y1, NULL);
		LineTo(device_context, x2, y1);
		LineTo(device_context, mid, y2);
		LineTo(device_context, x1, y1);
		x1 = x1 + 1, x2 = x2 - 1, y1 = y1 + 1, y2 = y2 - 1;
	}
	ReleaseDC(console_handle, device_context);
	DeleteObject(pen);
}

void myUDTriangle3(int x1, int y1, int x2, int y2, int R, int G, int B)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);

	//change the color by changing the values in RGB (from 0-255)
	HPEN pen = CreatePen(PS_SOLID, 2, RGB(R, G, B));
	SelectObject(device_context, pen);
	int mid = (x1 + x2) / 2;
	for (int i = 0; i < 25; i++)
	{
		MoveToEx(device_context, x1, y1, NULL);
		LineTo(device_context, x2, y1);
		LineTo(device_context, mid, y2);
		LineTo(device_context, x1, y1);
		x1 = x1 + 1, x2 = x2 - 1, y1 = y1 + 1, y2 = y2 - 1;
	}
	ReleaseDC(console_handle, device_context);
	DeleteObject(pen);
}

void cursor(int x1, int y1, int x2, int y2, int R, int G, int B)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);

	//change the color by changing the values in RGB (from 0-255)
	HPEN pen = CreatePen(PS_SOLID, 2, RGB(R, G, B));
	SelectObject(device_context, pen);
	MoveToEx(device_context, x1, y1, NULL);
	LineTo(device_context, x2, y1);
	LineTo(device_context, x2, y2);
	LineTo(device_context, x1, y2);
	LineTo(device_context, x1, y1);
	x1 = x1 + 6, x2 = x2 - 6, y1 = y1 + 8, y2 = y2 - 4;
	ReleaseDC(console_handle, device_context);
	DeleteObject(pen);
}